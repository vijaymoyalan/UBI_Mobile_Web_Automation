...
public class JiraCommonMethods {
    ...
    public static String login(){
        try {
            ...
            List<String> cookies = hm.get("Set-Cookie");
            if (cookies != null) {
                for (String cookieEntry : cookies) {
                    if (cookieEntry.contains("JSESSIONID")) {
                        JSESSIONID = cookieEntry.split(";")[0].split("=")[1];
                    } else if (cookieEntry.contains("atlassian.xsrf.token")) {
                        xsrfToken = cookieEntry.split(";")[0].split("=")[1];
                    }
                }
            }

            if (JSESSIONID == null || xsrfToken == null) {
                System.out.println("❌ Failed to extract valid JSESSIONID or XSRF token");
                return null;
            }

            String cookie = String.format("JSESSIONID=%s; atlassian.xsrf.token=%s", JSESSIONID, xsrfToken);
            System.out.println("✅ Cookie after fix: " + cookie);
            return cookie;
