Feature: Smoke of Borrowings.
  ...
  Scenario Outline: Verify getLoanDetails API for status code 200 with valid cif and cid
    ...
    Examples:
      |service                |endpoint        |requestBody                                                                       |requestField |responsefield     |filename    |responseBody|
      |loanserviceMSME  |getLoanDetails  |src\test\resources\testdata\api\request\Drop3\Borrowings\getLoanDetails.json|null					|null							 |null				|src\test\resources\testdata\api\response\expected\Drop3\Borrowings\getLoanDetails.json|
