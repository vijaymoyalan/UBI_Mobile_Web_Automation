Feature: Smoke for Registration using Debit card
...
@TestCaseKey=UDB-T1331
@TestCaseKey=UDB-T1332
Scenario Outline: Verify to validate debit card details using verifydebitcard_v1 API
Given user sets the environment of endpoint
Given user set the basepath to <service>
Then set the endpoint <endpoint>
Then user triggers a post request with <requestBody>
...
Examples:
|service     |endpoint            |requestBody                                                                 |responseBody|
|registration|/verifydebitcard_v1 |\src\test\resources\testdata\api\request\mvp\registration\verifydebitcard_v1.json |\src\test\resources\testdata\api\response\expected\mvp\registration\verifydebitcard_v1.json|
